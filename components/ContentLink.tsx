...
export function ContentLink({
  ...
  return (
    <Link
      href={href}
      className={cx(
        'block overflow-hidden rounded-2xl bg-white/5 md:p-7 p-4 shadow-surface-elevation-low transition duration-300 hover:bg-white/10 hover:shadow-surface-elevation-medium',
...
function Text({ children }: { children: React.ReactNode }) {
  return (
    <p className="mt-4 text-base md:text-lg text-rose-100/70 line-clamp-3">
      {children}
    </p>
  );
}
